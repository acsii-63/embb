...
private:
	...
	/******************************Subscriber***********************************/
	ros::Subscriber aruco_sub,whycon_sub,april_sub;
	...
	bool planner_terminated = false;
	std::vector<bool> marker_enabled,marker_activated;
	bool marker_terminated;
	std::vector<double> marker_range_max, marker_range_min;
	bool start = false;
	int marker_number,marker_number_total;
	...
	double height_coeff=0.4,safe_height = 2.0;
	...
	std::vector<marker_planner::terminate > markerTerminate;
	ros::ServiceClient setModeClient;
	std::vector <ros::ServiceClient> terminateClient;
...
public:
	...
	{	
		...
		marker_number_total = 3;

		terminateClient.resize(marker_number_total);

		markerTerminate.resize(marker_number_total);

		marker_activated.resize(3,false);
		...
		april_sub = nh.subscribe("/landing/marker_pose/whycon",1,&MarkerPlanner::marker_callback,this);
		...
		terminateClient[0] = nh.serviceClient<marker_planner::terminate>("/landing/marker_terminate/whycon");

		terminateClient[1] = nh.serviceClient<marker_planner::terminate>("/landing/marker_terminate/aruco");

		terminateClient[2] = nh.serviceClient<marker_planner::terminate>("/landing/marker_terminate/april");
	...
	void activate_maker(double height){
		for(int i=0; i<marker_number_total; i++){
			if(marker_enabled[i])
			{	
				bool inrange = false;

				if(height > marker_range_min[i] && height < marker_range_max[i]){
					inrange = true;
					// ROS_INFO_STREAM("height "<<height<<" i inrange " << i);
				}
				...
				if(inrange && !marker_activated[i]){
					markerTerminate[i].request.action=markerTerminate[i].request.start;
					terminateClient[i].call(markerTerminate[i]);
					if(markerTerminate[i].response.success == 1){
					marker_activated[i] = true;
					}
					else{
						int r2 = markerTerminate[i].response.success;
						ROS_ERROR_STREAM("cannot activate maker " << i << " result " << r2);
					}
				}
				if(!inrange && marker_activated[i]){
					markerTerminate[i].request.action=markerTerminate[i].request.stop;
					terminateClient[i].call(markerTerminate[i]);
					if(markerTerminate[i].response.success == 1)
					{
						marker_activated[i] = false;
					}
					else{
					ROS_ERROR_STREAM("cannot stop maker " << i);
					int r2 = markerTerminate[i].response.success;
					ROS_ERROR_STREAM("cannot stop maker " << i << " result "<< r2);
					}
				}
			}
			
		}
		int i1 = marker_activated[0];
		int i2 = marker_activated[1];
		int i3 = marker_activated[2];
		// ROS_INFO_STREAM("marker_activated "<<i1<<" "<<i2<<" "<<i3);
	}
	...
	void pose2Range(std::vector<geometry_msgs::Pose> parray){
		ROS_INFO("planner pose2range");
		marker_range_max.clear();
		marker_range_min.clear();
		marker_enabled.clear();
		for(auto p : parray){ 
			marker_range_max.push_back(p.position.x);
			marker_range_min.push_back(p.position.y);
			
			if(p.position.z > 0)
			{
				marker_enabled.push_back(true);
				marker_number++;
			}
			else
			{
				marker_enabled.push_back(false);
			}
		}
	}

	bool startCallback(controller_msgs::start::Request& req, controller_msgs::start::Response& res){
  		ROS_INFO("planner start callback");
  		if(!start){
  			seqID = req.seq;
			safe_height = req.sub;
			pose2Range(req.target.poses);
  			start = true ;
  			res.success = true;
  			res.result = 1;
			activate_maker(-999.0);
  			return true;
		ROS_INFO("planner start callback successful");
  		}
		ROS_INFO("planner start callback fail");
  		return false;
	}

	void marker_callback(const geometry_msgs::PoseWithCovariance &pose){
		if(start){
			Eigen::Quaterniond Base_camera = Eigen::Quaterniond(Eigen::AngleAxisd(M_PI, Eigen::Vector3d(0.70710678118,-0.70710678118,0.0)));
			Eigen::Vector3d M_Cam = toEigen(pose.pose.position);
			Eigen::Vector3d M_Base  = Base_camera * M_Cam;
			Eigen::Vector3d M_Inertia  = base_quaternion * M_Base;
			activate_maker(-M_Inertia(2));
			...
			if(M_Inertia(2) < -2){
				...
				safezone = true;
			int sf = safezone;
			ROS_INFO_STREAM("setpoint "<< Setpoint.x()<<" "<< Setpoint.y()<<" "<< Setpoint.z()<<" " <<  sf);
			...
			event_execute(Setpoint,pose.covariance[0]);
	...
	void plannerCallback(const ros::TimerEvent &event){
		switch (state_) {
			...
			case FINNISH : {
				...
				for(int i = 0; i < marker_number_total; i++){
					if(marker_enabled[i]){
						markerTerminate[i].request.action = markerTerminate[i].request.terminate;
      					terminateClient[i].call(markerTerminate[i]);
      					if(markerTerminate[i].response.success)
						{
						marker_enabled[i]=false;
						terminate_count++;
						}
					}
				}
				marker_number-=terminate_count;
	...
	void event_execute(Eigen::Vector3d &setpoint,double confidence){
		confidence = std::min(1.0,std::max(0.0,confidence));
		confidence = 1.0;
		Eigen::Vector3d discovarable = (setpoint - discovery)*confidence;
